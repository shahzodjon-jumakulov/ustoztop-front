...
const activeTab = ref(route.params.wallet ? wallets.value.findIndex(tab => tab.link == route.params.wallet[0]) : 0);
</script>
    ...
    <div class="max-lg:fixed max-lg:inset-0 max-lg:w-full max-lg:h-full bg-bg flex flex-col gap-4 sm:gap-5">
        <div class="flex p-4 sm:p-5 bg-white justify-center lg:hidden">
            ...
            <div class="flex gap-[5px] items-center">
                <span class="text-xl font-bold text-black">Кошелёк</span>
            </div>
        </div>
        <div class="bg-white rounded-3xl flex flex-col sm:mx-5 lg:hidden">
            <NuxtLink :to="localePath(`/profile/wallet/${item.link}`)"
                class="px-5 py-2.5 flex justify-between items-center h-[50px] hover:bg-bg2 active:bg-[#D2D7E5] rounded-full"
                v-for="item, index in wallets" :key="item.key" @click="activeTab = index">
                <div class="flex items-center gap-[5px]">
                    <span>{{ $t(`myWallet[${index}]`) }}</span>
                </div>
        ...
        <div class="flex gap-5 container">
            <div class="max-lg:hidden p-5 bg-white rounded-3xl flex flex-col w-[30%] h-fit">
                <NuxtLink :to="localePath(`/profile/wallet/${wallets[index].link}`)"
                    class="group px-2.5 flex gap-[5px] items-center h-[34px] hover:bg-bg active:bg-bg2 rounded-full cursor-pointer"
                    :class="{ 'active bg-bg': activeTab == index }" v-for="item, index in wallets" :key="item.key"
